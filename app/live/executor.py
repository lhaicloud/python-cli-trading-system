...
class LiveExecutor:
    ...
    def __init__(
        ...
        # `--yes` only skips the TTY confirm; it is never an Owner/Risk credential.
    ...
    def open_trade(
        ...
        Fail-closed Owner/Risk/LIVE_HALT gates run before any place_* order.
        `--yes` never satisfies those gates.
        """
        cfg = self._cfg
        with self._lock:
            presented_intent = trade_intent_id or getattr(sig, "trade_intent_id", None)
            auth = evaluate_live_entry_from_env(
                yes_flag=self._yes_flag,
                live_execution=True,
                trade_intent_id=presented_intent,
            )
            if not auth.allowed:
                logger.warning(
                    "[Executor] %s BLOCKED live-entry auth deny_code=%s ??? %s",
                    symbol, auth.deny_code, auth.reason,
                )
                return "blocked", None
    ...
    def handle_entry_limit_fill(
        ...
        # Fail-closed: do not add a live_trades row (or leave an unprotected
        # fill) if Owner/Risk/LIVE_HALT now deny new entries. Flatten instead.
        auth = evaluate_live_entry_from_env(
            yes_flag=self._yes_flag,
            live_execution=True,
            trade_intent_id=pending.get("trade_intent_id"),
        )
        if not auth.allowed:
            ok, reason = False, f"live-entry auth {auth.deny_code}"
        else:
