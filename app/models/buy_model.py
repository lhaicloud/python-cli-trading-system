"""BUY model wrapper — predict and score a potential long setup."""
...
def score_buy_setup(symbol: str, features: dict) -> float:
    """
    Return a probability score (0–1) for a BUY setup.

    Loads both the signal-based model ("buy") and the candle-based model
    ("buy_candle") if available. When both exist they are blended:
    60 % candle model (independent of rule engine) + 40 % signal model.
    Falls back to 0.0 if neither model is active.
    """
    signal_model = _get_model(symbol, "buy")
    candle_model = _get_model(symbol, "buy_candle")

    if signal_model is None and candle_model is None:
        return 0.0

    proba_signal = _predict(signal_model, features, "BUY/signal") if signal_model else 0.0
    proba_candle = _predict(candle_model, features, "BUY/candle") if candle_model else 0.0
...
def _get_model(symbol: str, model_type: str) -> dict | None:
    ...
    try:
        ...
        logger.warning("[BUY model] Load failed for %s/%s: %s", symbol, model_type, exc)
        return None
