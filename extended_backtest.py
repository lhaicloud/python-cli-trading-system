"""Extended multi-scenario backtest — 8 new market phases + ETH cross-asset."""
...
scenarios = [
    # ── New BTC scenarios ────────────────────────────────────────────────────
    ("BTC MEGA-BULL  2021-Q1", "BTCUSDT", "2021-01-01", "2021-03-31"),  # $29k→$58k
    ("BTC ATH-TOP    2021-Q4", "BTCUSDT", "2021-10-01", "2021-12-31"),  # $43k→$69k→$46k
    ("BTC EARLY-BEAR 2022-Q1", "BTCUSDT", "2022-01-01", "2022-03-31"),  # $47k→$32k
    ("BTC DEAD-CAT   2022-Q3", "BTCUSDT", "2022-07-01", "2022-09-30"),  # $19k→$25k→$19k
    ("BTC SIDEWAYS   2023-Q3", "BTCUSDT", "2023-07-01", "2023-09-30"),  # $30k→$25k
    ("BTC BREAKOUT   2024-Q1", "BTCUSDT", "2024-01-01", "2024-03-31"),  # $42k→$73k
    ("BTC PULLBACK   2024-Q2", "BTCUSDT", "2024-04-01", "2024-06-30"),  # $73k→$57k
    ("BTC BULL-ATH   2024-Q4", "BTCUSDT", "2024-10-01", "2024-12-31"),  # $60k→$107k
]
    ...
    try:
        ...
        if m['total_trades'] > 0:
            ...
            for t in trades[-15:]:
                ...
                p = '+' if t['pnl'] >= 0 else ''
                print(f"    {et} {t['direction']:4s} entry={t['entry_price']:.0f} => {p}${t['pnl']:.0f} [{t['exit_reason']}]  conf={t['signal_confidence']:.0f}")
        else:
...
# ── Summary table ─────────────────────────────────────────────────────────────
print(f"\n\n{'='*57}")
print(f"  EXTENDED BACKTEST SUMMARY")
print(f"{'='*57}")
...
total_pnl = sum(m['net_profit'] for _, m, _ in results if m['total_trades'] > 0)
profitable = sum(1 for _, m, _ in results if m['total_trades'] > 0 and m['net_profit'] > 0)
total = sum(1 for _, m, _ in results if m['total_trades'] > 0)
